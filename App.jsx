...
const ASSISTANT_R = {
  ...
  grillRedirect: { de: "🍽️ Einen \"Grillteller\" haben wir nicht direkt, aber unseren Kebap Teller ({price}) — mit Fleisch vom Drehspieß, Salat und Pommes oder Reis. Tippe unten, um ihn dir anzusehen.", en: "🍽️ We don't have a \"Grillteller\" as such, but our Kebap Teller ({price}) is close — with meat from the rotisserie, salad, and fries or rice. Tap below to check it out.", tr: "🍽️ Tam olarak \"Grillteller\" yok ama Kebap Teller'imiz ({price}) buna en yakını — döner eti, salata, patates veya pirinç ile. Görmek için aşağıya dokun.", ro: "🍽️ Nu avem exact un \"Grillteller\", dar Kebap Teller-ul nostru ({price}) e apropiat — cu carne de la frigare, salată și cartofi prăjiți sau orez. Apasă mai jos pentru a-l vedea.", nl: "🍽️ Een \"Grillteller\" hebben we niet precies, maar onze Kebap Teller ({price}) komt dichtbij — met vlees van het spit, salade en friet of rijst. Tik hieronder om te kijken.", sq: "🍽️ \"Grillteller\" nuk kemi saktësisht, por Kebap Teller-i ynë ({price}) është i ngjashëm — me mish nga hell-i, sallatë dhe patate ose oriz. Troko poshtë për ta parë.", ku: "🍽️ Me \"Grillteller\" tune, lê Kebap Teller-ê me ({price}) nêzîkî wê ye — bi goştê şîşê, selate û kartol an brincê. Li jêr bitikîne da ku bibînî.", pl: "🍽️ Nie mamy dokładnie \"Grillteller\", ale nasz Kebap Teller ({price}) jest podobny — z mięsem z rożna, sałatką i frytkami lub ryżem. Dotknij poniżej, aby zobaczyć." },
...
function getAssistantReply(qRaw, lang) {
  ...
  // gefragt (siehe Statistik), aber landet sonst im Fallback. Direkt auf
  // den nächstähnlichen Artikel (Kebap Teller) verweisen.
  if (has('grillteller', 'gegrilltes', 'grillplatte')) {
    const kebapTeller = ALL_MENU_ITEMS.find((it) => it.id === 'k13');
    if (kebapTeller) {
      return { intent: 'category', catKey: 'kebap', text: ar('grillRedirect', lang).replace('{price}', formatItemPriceText(kebapTeller)) };
    }
  }
